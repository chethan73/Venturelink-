...
//     '₹1Cr+',
//   ];
//   const sector = 'Investor'; // Fixed sector
...
//       setSubmittedData({ name, description, category, sector, fundingNeeds, file: file.name });
//       setShowPopup(true);

//       setTimeout(() => {
//         setShowPopup(false);
//         window.location.href = '/investor/investments';
//       }, 2000);
//     } catch (error) {
...
const ProposalSubmission = () => {
  ...
  const [description, setDescription] = useState('');
  const [category, setCategory] = useState('');
  const [sector, setSector] = useState('Investor');
  const [fundingNeeds, setFundingNeeds] = useState('');
  const [showPopup, setShowPopup] = useState(false);
  const [submittedData, setSubmittedData] = useState<any | null>(null);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full border border-slate-200 shadow-sm mb-4">
            ...
            <span className="text-sm font-medium text-slate-700">Investor Portal</span>
          </div>
          <h1 className="text-4xl font-bold text-slate-900 mb-3">Submit Your Proposal</h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Connect with investors by sharing your Investor details
          </p>
        </motion.header>
            ...
            >
              <div className="p-6 sm:p-8">
                ...
                <div className="mt-6 flex flex-col gap-3">
                  <button
                   onClick={() => router.push(`/investor/UploadPage/`)}
